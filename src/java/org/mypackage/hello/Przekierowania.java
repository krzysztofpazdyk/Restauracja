...
public class Przekierowania  {
    ...
    public String weryfikacjalogowania (Zalogowany zalogowany, HttpServletResponse response,HttpServletRequest request) throws IllegalAccessException, InvocationTargetException, SQLException, IOException {
        ...
        int kto=0;
        if (zalogowany.getLogin().equals("kucharz")) { kto=1;}
        if (zalogowany.getLogin().equals("kelner")) { kto=2;}
        if (zalogowany.getLogin().equals("dostawca")) { kto=3;}
        if (zalogowany.getLogin().equals("restaurator")) { kto=4;}
        if (zalogowany.getLogin().equals("zaopatrzenie")) { kto=5;}
        ...
        if (walidator.weryfikuj().equals("Nie")) {
            switch (kto) {
                ...
                case 3:
                    ...
                    break; 
                case 4:
                    response.sendRedirect("restauratordaniadowykonania.jsp");
                    break; 
                case 5:
